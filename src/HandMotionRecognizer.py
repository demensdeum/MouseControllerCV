from MotionRecognizer import MotionRecognizer
import cv2 as cv
import mediapipe as mp
import time

class HandMotionRecognizer(MotionRecognizer):
    ...
    def run(self, motionHandler):
        ...
        while True:
            ...
            try:
                ...
                pointer = detector.getPointer(img)
                print(fingers)

                pointerX = pointer[1]
                pointerY = pointer[2]

                xRatio = pointerX / 1920
                yRatio = pointerY / 1080
...
class HandDetector():
    ...
    def getPointer(self, img):
        lmList = self.findPosition(img, handNo = 0)
        min_element = min(lmList, key=lambda x: x[2])
